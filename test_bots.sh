#!/bin/bash
for file in example_bots/*.jar my_other_bots/*.jar
...
do
   ...
   for i in {1..100}
   do
      RES=`java -jar tools/PlayGame.jar maps/map$i.txt 500 500 log.txt "java -jar $file" "python bot.py" 2>&1 | tail -n 3 | grep "Turn\|Player"`
   ...
   echo "won against $file : $player_1_counter/$maps_played, avg turns: $player_1_turn_counter"
   echo "lost against $file lost : $player_2_counter/$maps_played, avg turns: $player_2_turn_counter"
done